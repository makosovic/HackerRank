...
namespace PacmanUCS
{
    ...
    class Solution
    {
        ...
        private static void Astar(State state)
        {
            ...
            FoodNode = new Node<Position<int>>(state.FoodPosition);
        ...
        private static void Explore(State state, Node<Position<int>> parentNode)
        {
            ...
            if (!Expanded.Any(x => x.I == state.PacmanPosition.I && x.J == state.PacmanPosition.J))
                Explored.Enqueue(new ExploredPosition(state.PacmanPosition, parentNode), parentNode.Depth + 1 + DistanceToFood(state.PacmanPosition));
        }
        ...
        private static int DistanceToFood(Position<int> pacmanPosition)
        {
            return Math.Abs(pacmanPosition.I - FoodNode.Value.I) + Math.Abs(pacmanPosition.J - FoodNode.Value.J);
        }
